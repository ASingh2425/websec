...
export const ScanForm: React.FC<ScanFormProps> = ({ onScan, isScanning }) => {
  ...
  const refreshPlan = () => {
      ...
      // Auto-select defaults based on plan. If selected model isn't allowed, switch to lite or flash.
      if (!p.allowedModels.includes(model)) {
          if (p.allowedModels.includes('lite')) setModel('lite');
          else setModel('flash');
      }
      
      // Smart Validation: Default to Deep if current setting is invalid or if user just landed
