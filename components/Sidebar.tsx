      ...
      `}>
        <div className="h-20 px-6 flex items-center justify-between border-b border-cyber-border/50">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => onNavigate('scanner')}>
            ...
            <div>
               <h1 className="text-xl font-display font-extrabold text-cyber-text-main tracking-tight leading-none">WebSec</h1>
        ...
        <div className="px-4 py-8 space-y-8 flex-1 overflow-y-auto custom-scrollbar">
          <div className="bg-cyber-cardHighlight/50 border border-cyber-border rounded-xl p-4">
              ...
              {plan.id === 'free' && (
                  <button onClick={() => setShowPricing(true)} className="w-full mt-3 flex items-center justify-center gap-2 bg-cyber-primary hover:bg-cyber-primaryEnd text-white text-xs font-bold py-2 rounded-lg transition-colors shadow-glow">
        ...
        <div className="p-4 border-t border-cyber-border bg-cyber-cardHighlight/30">
           <button onClick={onLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-cyber-text-secondary hover:bg-rose-500/10 hover:text-rose-400 transition-all border border-transparent hover:border-rose-500/20 group">
              <LogOut size={18}/> 
